...
#### metal levels 
#### Part 2: Multi metals DeSeq2 analysis, with index variables, asjusted 
####
...
Genes_backgroundfiltered <- countdata %>% 
  ...
  group_by(Gene) %>% 
  dplyr::summarize(totalabovemedian=sum(abovemedian)) %>% 
...
# For the this analysis, we are interesting in keeping index variables, as well as the following covariates:
# score (SES cumulative index), bmicat (maternal prepreg BMI), smoke (maternal smoking during pregnancy) and SVs
colnames(coldata)
coldata <- coldata %>% select(ID, score, bmicat, smoke, sex, SV1, SV2, SV3, PC1, PC2, PC3, PC4, toxicindex, essentialindex)
...
coldata[pcvars] <- lapply(coldata[pcvars], as.numeric)
coldata[indexvars] <- lapply(coldata[indexvars], as.numeric)
...
#Create the dds objects for all the index and PC variables 
PC1 <- DESeqDataSetFromMatrix(countData = countdata,
                                     colData = coldata,
                                     design = ~SV1 + SV2+ score + bmicat + smoke + sex + PC1)
PC2 <- DESeqDataSetFromMatrix(countData = countdata,
                                     colData = coldata,
                                     design = ~SV1 + SV2+ score + bmicat + smoke + sex + PC2)
PC3 <- DESeqDataSetFromMatrix(countData = countdata,
                                     colData = coldata,
                                     design = ~SV1 + SV2+ score + bmicat + smoke + sex + PC3)
PC4 <- DESeqDataSetFromMatrix(countData = countdata,
                                     colData = coldata,
                                     design = ~SV1 + SV2+ score + bmicat + smoke + sex + PC4)
toxicindex <- DESeqDataSetFromMatrix(countData = countdata,
                              colData = coldata,
                              design = ~SV1 + SV2+ score + bmicat + smoke + sex + toxicindex)
essentialindex <- DESeqDataSetFromMatrix(countData = countdata,
                                colData = coldata,
                                design = ~SV1 + SV2+ score + bmicat + smoke + sex + essentialindex)
...
for (i in 1:length(pcvars)){
  dds <- estimateSizeFactors(eval(parse(text = paste0(pcvars[[i]]))))
  sizeFactors(dds)
  assign(paste0(pcvars[[i]]),dds) 
  print(pcvars[[i]])
}
for (i in 1:length(indexvars)){
  dds <- estimateSizeFactors(eval(parse(text = paste0(indexvars[[i]]))))
  sizeFactors(dds)
  assign(paste0(indexvars[[i]]),dds) 
  print(indexvars[[i]])
}
...
for (i in 1:length(pcvars)){
  vsd <- varianceStabilizingTransformation(eval(parse(text = paste0(pcvars[[i]]))), blind=FALSE)
  vsd_matrix <-as.matrix(assay(vsd))
  write.csv(vsd_matrix, paste0(Output_Folder,"/", cur_date,"_", pcvars[[i]], "_VSDCounts_IncludedSubjects.csv"), row.names=TRUE)
  assign(paste0("vsd_matrix.",pcvars[[i]]),vsd_matrix) 
  print(pcvars[[i]])
}
for (i in 1:length(indexvars)){
  vsd <- varianceStabilizingTransformation(eval(parse(text = paste0(indexvars[[i]]))), blind=FALSE)
  vsd_matrix <-as.matrix(assay(vsd))
  write.csv(vsd_matrix, paste0(Output_Folder,"/", cur_date,"_", indexvars[[i]], "_VSDCounts_IncludedSubjects.csv"), row.names=TRUE)
  assign(paste0("vsd_matrix.",indexvars[[i]]),vsd_matrix) 
  print(indexvars[[i]])
}
...
plotPCA(vsd, intgroup="PC1", returnData=FALSE)
...
for (i in 1:length(pcvars)){
  dds <- DESeq(eval(parse(text = paste0(pcvars[[i]]))), betaPrior=FALSE)  # because we used a user-defined model matrix, need to set betaPrior=FALSE
  res <- results(dds, pAdjustMethod = "BH") #with multiple test correction by the default, BH (aka FDR)
  write.csv(as.data.frame(res)[order(res$padj),], paste0(Output_Folder,"/", cur_date,"_", pcvars[[i]],"_AllStatResults.csv"))# Exporting statistical results
  assign(paste0("res.",pcvars[[i]]),res) 
  print(pcvars[[i]])
}
for (i in 1:length(indexvars)){
  dds <- DESeq(eval(parse(text = paste0(indexvars[[i]]))), betaPrior=FALSE)  # because we used a user-defined model matrix, need to set betaPrior=FALSE
  res <- results(dds, pAdjustMethod = "BH") #with multiple test correction by the default, BH (aka FDR)
  write.csv(as.data.frame(res)[order(res$padj),], paste0(Output_Folder,"/", cur_date,"_", indexvars[[i]],"_AllStatResults.csv"))# Exporting statistical results
  assign(paste0("res.",indexvars[[i]]),res) 
  print(indexvars[[i]])
}
...
for (i in 1:length(pcvars)){
  ...
  MA <- as.data.frame(eval(parse(text = paste0("res.",pcvars[[i]]))))[order(eval(parse(text = paste0("res.",pcvars[[i]])))$padj),]
  MA[is.na(MA)] <- 1
  ...
  plot <-
    ...
    theme_bw()+ theme(axis.text.x = element_text(size = 2), axis.text.y = element_text(size=2)) +
    ...
    ggtitle(paste0(pcvars[[i]])) +
    theme(plot.title = element_text(hjust = 0.5))
  
  assign(paste0("plot.",pcvars[[i]]),plot) 
}

print(pcvars)
all_pc <- ggarrange(plot.PC1, plot.PC2, plot.PC3, plot.PC4,
                     nrow=2, ncol=2)

all_pc

pdf(paste0(Output_Folder, "/", cur_date, "_","PCs", "_MAplot.pdf"),
    width = 10, height = 10)
all_pc
dev.off()
...
for (i in 1:length(indexvars)){
  ...
  MA <- as.data.frame(eval(parse(text = paste0("res.",indexvars[[i]]))))[order(eval(parse(text = paste0("res.",indexvars[[i]])))$padj),]
  MA[is.na(MA)] <- 1
  MA1 <- MA[ which(MA$padj>=0.1),]  # all the rest
  MA2 <- MA[ which(MA$padj<0.1 & MA$log2FoldChange > log2(1.5)),]   # sig up-regulated w/ FC filter-FIREBRICK
  MA3 <- MA[ which(MA$padj<0.1 & MA$log2FoldChange < -log2(1.5)),]  # sig down-regulated w/ FC filter-dodgerblue4
  ...
  plot <-
    ...
    ylab("(log2)FC, each log unit increase") +
    geom_hline(yintercept=0)+
    ggtitle(paste0(indexvars[[i]])) +
    theme(plot.title = element_text(hjust = 0.5))
  
  assign(paste0("plot.",indexvars[[i]]),plot) 
}

print(indexvars)
all_index <- ggarrange(plot.toxicindex, plot.essentialindex, 
                    nrow=1, ncol=2)

all_pc

pdf(paste0(Output_Folder, "/", cur_date, "_","indexs", "_MAplot.pdf"),
    width = 10, height = 5)
all_index
dev.off()
...
#PC1
  siggenes <- as.data.frame(res.PC1)[order(res.PC1$padj),]
  ...
  FC_pvalue1 <- EnhancedVolcano(res.PC1,
                                 lab = rownames(res.PC1),
    ...
    FC_pvalue1 <- FC_pvalue1 + ggtitle("PC1") +theme(title = element_text(size=5))
    FC_pvalue1

    
#PC2
    siggenes <- as.data.frame(res.PC2)[order(res.PC2$padj),]
    siggenes <- as.data.frame(siggenes) %>% 
      rownames_to_column(var="gene") %>% 
      filter(padj <0.1) %>% 
      mutate(gene=as.character(gene)) 
    siggenes <- (as.vector(siggenes$gene))[1:30]
    siggenes<-c(siggenes,"EIF1AXP1","EIF2A","EIF3I","DNMT1","IGFBP1")
    #selectLab_italics = paste0("italic('",c(siggenes),"')")
    FC_pvalue2 <- EnhancedVolcano(res.PC2,
                                  lab = rownames(res.PC2),
                                  x = 'log2FoldChange',
                                  y = 'padj',
                                  selectLab = c(siggenes),
                                  pCutoff= 0.1,
    ...
    FC_pvalue2 <- FC_pvalue2 + ggtitle("PC2") +theme(title = element_text(size=5))
    FC_pvalue2  
    

pdf(paste0(Output_Folder, "/", cur_date, "_","PCs", "_volcanoplot_labels.pdf"),
        width = 6, height = 7)
FC_pvalue2
dev.off()    
    
    
FC_pvalue2a <- EnhancedVolcano(res.PC2,
                              lab = rownames(res.PC2),
                              x = 'log2FoldChange',
                              y = 'padj',
                              selectLab = c(siggenes),
...
FC_pvalue2a <- FC_pvalue2a + ggtitle("PC2") +theme(title = element_text(size=5))
FC_pvalue2a  


pdf(paste0(Output_Folder, "/", cur_date, "_","PCs", "_volcanoplot_nolabelsnoFCcutoff.pdf"),
    width = 6, height = 7)
FC_pvalue2a
dev.off()    

#toxicindex
siggenes <- as.data.frame(res.toxicindex)[order(res.toxicindex$padj),]
siggenes <- as.data.frame(siggenes) %>% 
  rownames_to_column(var="gene") %>% 
  filter(padj <0.1) %>% 
  mutate(gene=as.character(gene)) 
...
FC_pvaluetoxic <- EnhancedVolcano(res.toxicindex,
                              lab = rownames(res.toxicindex),
                              selectLab = c(siggenes),
                              x = 'log2FoldChange',
                              y = 'padj',
                              pCutoff= 0.1,
...
FC_pvaluetoxic <- FC_pvaluetoxic + ggtitle("Toxic index") +theme(title = element_text(size=5))
FC_pvaluetoxic


pdf(paste0(Output_Folder, "/", cur_date, "_","volcanoplot_toxicindex.pdf"),
    width = 6, height = 7)
FC_pvaluetoxic
dev.off()  

#essentialindex
siggenes <- as.data.frame(res.essentialindex)[order(res.essentialindex$padj),]
siggenes <- as.data.frame(siggenes) %>% 
  rownames_to_column(var="gene") %>% 
  filter(padj <0.1) %>% 
  mutate(gene=as.character(gene)) 
...
FC_pvalueessential <- EnhancedVolcano(res.essentialindex,
                                  lab = rownames(res.essentialindex),
                                  selectLab = c(siggenes),
                                  x = 'log2FoldChange',
                                  y = 'padj',
                                  pCutoff= 0.1,
...
FC_pvalueessential <- FC_pvalueessential + ggtitle("Essential index") +theme(title = element_text(size=5))
FC_pvalueessential


pdf(paste0(Output_Folder, "/", cur_date, "_","volcanoplot_essentialindex.pdf"),
    width = 6, height = 7)
FC_pvalueessential
dev.off()  
...
all_pcs <- ggarrange(FC_pvalue1, FC_pvalue2,
                     nrow=1, ncol=2, common.legend=TRUE)

all_pcs

pdf(paste0(Output_Folder, "/", cur_date, "_","PCs", "_volcanoplot.pdf"),
    width = 12, height = 7)
all_pcs
dev.off()

for (i in 1:length(indexvars)){
  siggenes <- as.data.frame(eval(parse(text = paste0("res.",indexvars[[i]]))))[order(eval(parse(text = paste0("res.",indexvars[[i]])))$padj),]
  siggenes <- as.data.frame(siggenes) %>% 
    rownames_to_column(var="gene") %>% 
    filter(padj <0.1) %>% 
    mutate(gene=as.character(gene)) 
  ...
  FC_pvalue <- EnhancedVolcano(eval(parse(text = paste0("res.",indexvars[[i]]))),
                               lab = rownames(eval(parse(text = paste0("res.",indexvars[[i]])))),
                               selectLab = c(siggenes),
                               x = 'log2FoldChange',
                               y = 'padj',
                               pCutoff= 0.1,
  ...
  FC_pvalue <- FC_pvalue + ggtitle(paste0(indexvars[[i]])) +theme(title = element_text(size=5))
  
  assign(paste0("plotvolcano.",indexvars[[i]]),FC_pvalue)
}
print(indexvars)
all_index <- ggarrange(plotvolcano.essentialindex, plotvolcano.toxicindex, nrow=1, ncol=2, common.legend=TRUE)

all_index

pdf(paste0(Output_Folder, "/", cur_date, "_","toxicandessentialindex", "_volcanoplot.pdf"),
    width = 10, height = 5)
all_index
dev.off()
...
#################################################################################################
#################################################################################################
#### Save eveything in environment that might be useful in future analyses
#################################################################################################
#################################################################################################
...
deseqobj_pcvars <- mget(pcvars)
deseqobj_indexvars <- mget(indexvars)
plots <-mget(ls(pattern = "plot"))

save(allres, allsvseqs, allvsd,deseqobj_pcvars,deseqobj_indexvars,plots,
